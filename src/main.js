...
import { Jupiter, Mars, Mercury, Neptune, Saturn, Uranus, Venus } from "./planets/Planet.ts";
...
scene.background = new TextureLoader().load("/assets/bg.jpg");
...
function renderButtonsList(list) {
...
function renderPlanetList(list) {
    const ui = list.map((x) => `<div class="p-3 bg-gray-100 rounded-xl border-2">
    ...
    ui.forEach((item, i) => {
        ...
        remove?.addEventListener("click", () => {
            planets = list.filter((x) => x.name != list[i].name);
...
let spaceShip = null;
loader.load('/assets/ship.gltf', // Path to your model file
(gltf) => {
    const m = gltf.scene;
    scene.add(m);
    spaceShip = m;
...
function moveAndRotateSpaceShip() {
    if (spaceShip) {
        ...
        if (moveForward)
            spaceShip.position.add(direction.multiplyScalar(0.5));
        if (moveBackward)
            spaceShip.position.add(direction.multiplyScalar(-0.5));
        if (rotateLeft) {
